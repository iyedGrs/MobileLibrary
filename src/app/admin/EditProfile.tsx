...
const EditBook = () => {
  ...
  const { bookId } = useLocalSearchParams(); // Récupère l'ID du livre depuis les paramètres
  const [bookTitle, setBookTitle] = useState<string>(""); // Titre à éditer
  const [bookAuthor, setBookAuthor] = useState<string>(""); // Auteur du livre
  ...
  useEffect(() => {
    if (bookId) {
      const book = books.find((b) => b.id === bookId); // Recherche du livre par ID
      if (book) {
