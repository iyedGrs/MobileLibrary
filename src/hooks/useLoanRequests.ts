...
import { useState, useEffect } from "react"
import { fetchPendingLoans, acceptLoan, rejectLoan, cancelLoan } from "../services/loanService"
...
export const useLoanRequests = () => {
  ...
  const acceptLoanRequest = async (loanId: string) => {
    ...
    try {
      const updatedLoan = await acceptLoan(loanId)
      setRequests((prevRequests) => prevRequests.map((request) => (request.id === loanId ? updatedLoan : request)))
    } catch (error) {
      setError("Failed to accept the request. Please try again.")
    } finally {
      setIsLoading(false)
  ...
  // Annuler une demande
  const cancelLoanRequest = async (loanId: string) => {
    setIsLoading(true)
    setError(null)
    try {
      const updatedLoan = await cancelLoan(loanId)
